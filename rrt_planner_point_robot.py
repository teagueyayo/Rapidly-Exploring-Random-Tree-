...
def takeStep(xStart, xGoal):
    ...
    xNew = [xStart[0] + SMALLSTEP*math.cos(theta), xStart[1] + SMALLSTEP*math.sin(theta)]
    return xNew
...
def rrt_search(G, tx, ty):
    ...
    while (1) :
        ...
        if (obstacleFree(vNearest, vNew) != 0):
            node = pointToVertex(vNew)
            G[nodes].append(node)
            G[edges].append((xNearest, node))
            new_edges.append((xNearest, node))
            counter += 1
            if(counter % 100 == 0):
                drawNewEdges()
                canvas.events()
